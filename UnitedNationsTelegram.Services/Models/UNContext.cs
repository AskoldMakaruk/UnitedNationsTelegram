...
namespace UnitedNationsTelegram.Services.Models;
...
public class UNContext : IdentityDbContext<UNUser>
{
    ...
    public async Task<List<UserCountry>> MainMembers(long chat)
    {
        return UserCountries.Include(a => a.Votes)
            .Include(a => a.Country)
            .Include(a => a.User)
            .Where(a => a.ChatId == chat).AsEnumerable().OrderByDescending(a => a.Votes.Count(a => a.Created > DateTime.Now.StartOfWeek(DayOfWeek.Monday))).Take(Constants.MainMembersCount).ToList();
    }
